...
Contains only a `.gitkeep` placeholder. The actual `nutrition_data.duckdb` file is generated by `build_nutrition_db.py` and gitignored. The DB holds a single wide table (`nutrition_data`) with ~46 columns and ~3.6M rows.
...
**Interaction:** Imports `run_single_question`, `save_single_trace`, `warmup_runtime` from `src.nutrition_sql.service`. Connects to DuckDB at `DB_PATH` (default: `database/nutrition_data.duckdb`).
...
| **Admin hierarchy** | `state_id`, `district_id`, `project_id`, `sector_id`, `awc_id`, `awc_code` | INT/VARCHAR | Anganwadi centre geographic hierarchy |
...
**Total rows:** ~3.6M (from `cleaned_dataset_with_labels.csv`).
